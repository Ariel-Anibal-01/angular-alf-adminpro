...
export class EditarPublicacionComponent implements OnInit {
  ...
  constructor( private fb:FormBuilder,private publicacionService: PublicacionService, private router: Router, private route: ActivatedRoute) {
  ...
  ngOnInit(): void {
    ...
    .subscribe(  (resp ) => {
       ...
       const p : Publicacion = resp['publicationDB'];
      ...
      this.bodies = p.bodies;
      this.date = new Date(p.date);
      this.slug = p.slug;
      this.idPublication = p._id;
  ...
  //  this.formData.append('myFile', this.imagePortada);
    ...
    this.publicacionService.editarPublicacion(this.idPublication,this.formData)
    .subscribe( resp => {


      Swal.fire('Publicación editada', this.title, 'success').then(() => {
        this.router.navigate(['./dashboard/publicaciones']);
      });
    })
